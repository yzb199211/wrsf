package com.yyy.wrsf.utils.net.order;

public class OrderUrl {
    public final static String insertContractInfo = "/contractInfo/insertContractInfo";
    public final static String getPageList = "/contractInfo/getPageList";
    public final static String getCompanyPageList = "/contractInfo/getCompanyPageList";
    public final static String getAllCompanyPageList = "/contractInfo/getAllCompanyPageList";
    public final static String getDaiPageList = "/contractInfo/getDaiPageList";
//    public final static String cancelOrder = "/contractInfo/cancel";
    public final static String cancelOrder = "/contractInfo/shopConfirm";
    public final static String shopRecConfirm = "/contractInfo/shopRecConfirm";
    public final static String SendNoticeByCustomer = "/contractInfo/SendNoticeByCustomer";
    public final static String getMyRecPageList = "/contractInfo/getMyRecPageList";
    public final static String shopConfirm = "/contractInfo/shopConfirm";
}
