...
public class OrderNoticeP implements IOrderP {
    ...
    private void initParams() {
        pager.setPageIndex(pageIndex);
        pager.setQueryParam(iOrderM.getFilterNotice(iOrderV.getOrderName(), iOrderV.getType()));
    }
    ...
    public void notice(int pos, String id) {
        ...
        iOrderM.Requset(cancelParams(id), NetConfig.address + OrderUrl.SendNoticeByCustomer, RequstType.GET, new OnResultListener() {
            ...
            public void onSuccess(String data) {
                if (!destroyFlag) {
                    handler.post(() -> {
                        iOrderV.finishLoading(BaseApplication.getInstance().getString(R.string.common_noticce_success));
                        iOrderV.setItemType(pos, ContractStatusEnum.CANCEL.getStatus());
                        iOrderV.refreshList();
                    });
                }
